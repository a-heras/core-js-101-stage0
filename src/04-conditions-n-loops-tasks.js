...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${openingBracket}${start}, ${end}${closingBracket}`;
}
...
function isBracketsBalanced(str) {
  const brackets = { ')': '(', ']': '[', '}': '{', '>': '<' };
  const stack = [];
  const isInvalid = str.split('').some((char) => {
    if (Object.values(brackets).includes(char)) {
      stack.push(char);
      return false;
    }
    if (Object.keys(brackets).includes(char)) {
      return stack.pop() !== brackets[char];
    }
    return false;
  });
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const val = position[r1][c1];

    if (val && val === position[r2][c2] && val === position[r3][c3]) {
      return val;
    }
  }
